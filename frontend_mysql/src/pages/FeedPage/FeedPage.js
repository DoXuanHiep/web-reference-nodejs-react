    ...
    return (
        <Fragment>
            ...
            {isOpenEditPostPage && <EditPost onCancel={closeEditPostHandler} 
                id={post.id} title={post.title} image={post.imageUrl} content ={post.content} /> }
            {isPending ? <p>Pending</p> : <></>}
            <div className={classes.btn}>
                <button onClick={openCreatePostPageHandler}>New Post</button>
            </div>
            {posts.map(post => (
                <PostsList key={post.id} post= {post} onEdit ={openEditPostPageHandler} onDelete = {deletePostHandler}/>
            ))}
            <div className={classes.btn}>
